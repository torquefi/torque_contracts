...
const { ZERO_ADDRESS } = require("./StargateUtils/constants");
const {
  getAddr,
  deployNew,
  getCurrentBlock,
...
describe("LPStaking:", function () {
  ...
  it("constructor() - reverts for bad params", async function () {
    await expect(
      deployNew("LPStaking", [mockToken.address, emissionsPerBlock, 0, 1])
    ).to.be.revertedWith("LPStaking: _startBlock must be >= current block");
    ...
    ).to.be.revertedWith("Stargate: _stargate cannot be 0x0");
  });
  ...
  it("deposit()", async function () {
    ...
    await expect(lpStaking.withdraw(poolId, depositAmt.mul(3))).to.emit(
      lpStaking,
      "Withdraw"
    );
  ...
  it("deposit()", async function () {
    ...
    const pendingStargate = await lpStaking.pendingStargate(
      poolId,
      owner.address
    );
    ...
    await lpStaking.updatePool(poolId);
    await mineNBlocks(2);
    const _pendingStargate = await lpStaking.pendingStargate(
      poolId,
      owner.address
    );
  ...
  it("emergencyWithdraw() - changes the lp amount stored in lpBalance", async function () {
    ...
    await mockToken
      .connect(alice)
      .approve(lpStaking.address, "100000000000000000000000");

    // emergencyWithtdraw() from the pool
    let depositAmt2 = BigNumber.from("7000000000000000000");
  ...
  it("add() - reverts with 0x0 token", async function () {
    await expect(lpStaking.add(allocPoint, ZERO_ADDRESS)).to.revertedWith(
      "StarGate: lpToken cant be 0x0"
    );
  });
  ...
  it("withdraw() - withdraws if amount is too large", async function () {
    ...
    await expect(
      lpStaking.withdraw(poolId, "4000000000000000000")
    ).to.be.revertedWith("withdraw: _amount is too large");
  });
  ...
  it("withdraw() - withdraw exceeds the amount owned by the lp contract", async function () {
    ...
    expect(await stargateToken.balanceOf(owner.address)).to.equal(
      userBal.add(amountToSend)
    );
  });
  ...
  it("getMultiplier() - _to is > bonusEndblock and _from is < bonusEndblock", async function () {
    ...
    await expect(result).to.equal(
      bonusEndBlock.sub(from).mul(bonusMultiplier).add(to.sub(bonusEndBlock))
    );
  });

  it("setStargatePerBlock() - reverts when non owner", async function () {
    await expect(
      lpStaking.connect(badUser1).setStargatePerBlock(0)
  ...
  it("updatePool() - lpSupply is 0", async function () {
    ...
    await lpStaking.updatePool(poolId);
    const { lastRewardBlock: _lastRewardBlock } = await lpStaking.poolInfo(
      poolId
    );
