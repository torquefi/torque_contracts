...
contract Router is IStargateRouter, Ownable, ReentrancyGuard {
    ...
    function sendCredits(
        ...
        Pool pool = _getPool(_srcPoolId);
        Pool.CreditObj memory c = pool.sendCredits(_dstChainId, _dstPoolId);
        bridge.sendCredits{ value: msg.value }(
            _dstChainId,
            _srcPoolId,
            _dstPoolId,
            _refundAddress,
    ...
    function revertRedeemLocal(
        ...
        bridge.redeemLocalCallback{ value: msg.value }(
            _dstChainId,
            _refundAddress,
            c,
            _lzTxParams,
    ...
    function retryRevert(
        ...
        if (functionType == TYPE_REDEEM_LOCAL_CALLBACK_RETRY) {
            (
                ...
                uint256 amountSD,
                uint256 mintAmountSD
            ) = abi.decode(payload, (uint8, uint256, uint256, address, uint256, uint256));
            _redeemLocalCallback(
                _srcChainId,
                _srcAddress,
                _nonce,
                srcPoolId,
    ...
    function _redeemLocalCallback(
        ...
        Pool pool = _getPool(_dstPoolId);
        try
            pool.redeemLocalCallback(_srcChainId, _srcPoolId, _to, _amountSD, _mintAmountSD)
        {} catch {
            revertLookup[_srcChainId][_srcAddress][_nonce] = abi.encode(
