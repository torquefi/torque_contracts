    ...
    function lzReceive(
        ...
        if (functionType == TYPE_SWAP_REMOTE) {
            ...
            (
                ...
                uint256 amountSD,
                bytes memory to
            ) = abi.decode(_payload, (uint8, uint256, uint256, Pool.CreditObj, uint256, bytes));
            router.creditChainPath(_srcChainId, srcPoolId, dstPoolId, c);
            router.redeemLocalCheckOnRemote(
                _srcChainId,
                _srcAddress,
                _nonce,
                srcPoolId,
    ...
    function redeemLocalCallback(
...

        {
            (
                ...
                uint256 mintAmountSD,
                bytes memory to
            ) = abi.decode(_payload, (uint8, uint256, uint256, uint256, uint256, bytes));
