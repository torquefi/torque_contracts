...
contract StargateFeeLibraryV02 is IStargateFeeLibrary, Ownable, ReentrancyGuard {
    ...
    function getFees(
        ...
        // calculate the equilibrium fee
        (uint256 eqFee, uint256 protocolSubsidy) = _getEquilibriumFee(
        ...
        uint256 currentAssetSD = IERC20(tokenAddress).balanceOf(address(pool)).div(
            pool.convertRate()
        );
    ...
    function _getEquilibriumFee(
        ...
        if (afterBalance >= safeZoneMax) {
            ...
            if (beforeBalance >= safeZoneMin) {
                ...
                // part 2
                eqFee = eqFee.add(
                    _getTrapezoidArea(LAMBDA_2, LAMBDA_1, safeZoneMin, 0, safeZoneMin, afterBalance)
                );
            } else {
                // only in danger zone
                // part 2 only
