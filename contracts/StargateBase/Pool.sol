...
contract Pool is LPTokenERC20, ReentrancyGuard {
    ...
    function swap(
        ...
        SwapObj memory s = IStargateFeeLibrary(feeLibrary).getFees(
            poolId,
            _dstPoolId,
            _dstChainId,
            _from,
    ...
    function setDeltaParam(
        ...
        require(
            _swapDeltaBP <= BP_DENOMINATOR && _lpDeltaBP <= BP_DENOMINATOR,
            "Stargate: wrong Delta param"
        );
        batched = _batched;
                ...
                if (fullMode) {
                    ...
                    for (uint256 i = 0; i < cpLength; ++i) {
                        if (deficit[i] > 0) {
                            ...
                            uint256 amtToCredit = deficit[i].add(
                                excessCredit.mul(cp.weight).div(totalWeight)
                            );
                            spent = spent.add(amtToCredit);
                            cp.credits = cp.credits.add(amtToCredit);
                        }
