...
contract BTCBorrow is UUPSUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function borrow(
        ...
        uint maxBorrow = (supplyAmount.add(userBorrowInfo.supplied))
    ...
    function withdraw(uint withdrawAmount) public nonReentrant {
        ...
        uint minRequireSupplyAmount = userBorrowInfo
            .borrowed
            .mul(SCALE)
            .mul(PRICE_MANTISA)
            .div(price)
            .div(uint(info.borrowCollateralFactor).sub(WITHDRAW_OFFSET));
    ...
    function repay(uint usdRepayAmount) public nonReentrant {
        ...
        uint withdrawAssetAmount = userBorrowInfo.supplied.mul(withdrawUsdcAmountFromEngine).div(
            userBorrowInfo.borrowed
        );
        ...
        if (userBorrowInfo.baseBorrowed < withdrawUsdcAmountFromEngine) {
            ...
            userBorrowInfo.baseBorrowed = userBorrowInfo.baseBorrowed.sub(
                withdrawUsdcAmountFromEngine
            );
        }
        ...
        require(
            ERC20(asset).transfer(msg.sender, withdrawAssetAmount),
            "Transfer asset from Compound failed"
        );
    }
    ...
    function calculateInterest(uint borrowAmount, uint borrowTime) public view returns (uint) {
        ...
        return
            borrowAmount.mul(icomet.getBorrowRate(icomet.getUtilization())).mul(totalSecond).div(
                1e18
            );
    }
