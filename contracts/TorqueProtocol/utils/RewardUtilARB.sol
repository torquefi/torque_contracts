...
/// @title RewardUtilARB
/// @notice This contract manages the reward distribution for users within the Torque ecosystem based on their deposits and borrowings, specifically using ARB tokens.
/// @dev The contract supports multiple Torque contracts and allows for reward claiming, reward configuration updates, and reward calculations.
contract RewardUtilARB is ReentrancyGuard, Ownable { 
    ...
    /// @notice The ARB token used for rewards
    ...
    /// @notice Constructor to initialize the contract with the default ARB token and governor
    ...
    /// @notice Changes the ARB token address
    /// @param _arbToken The new ARB token address
    ...
    function claimReward(address[] memory _torqueContract) external {
        ...
        require(arbToken.transfer(msg.sender, rewardAmount), "Transfer failed");
    }
    ...
    /// @notice Allows the owner to withdraw ARB tokens from the contract
    /// @param _amount The amount of ARB to withdraw
    function withdrawArb(uint256 _amount) external onlyOwner {
        require(arbToken.transfer(msg.sender, _amount), "Transfer failed");
    }
