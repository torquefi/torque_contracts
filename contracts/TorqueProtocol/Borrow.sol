...
// End test
    ...
    function borrow(uint amount, uint amountBorrow) public payable nonReentrant{
        ...
        uint price = comet.getPrice(info.priceFeed);
        uint maxBorrow = amount.mul(info.borrowCollateralFactor).mul(price).div(SCALE);

        BorrowInfo userBorrowInfo = borrowInfoMap[msg.sender];
        uint borrowable = maxBorrow.sub(userBorrowInfo.borrowed);
        require(borrowable > amount, "borrow exceed");
        require(ERC20(token_).transferFrom(msg.sender,address(this), amount), "transfer asset fail");
